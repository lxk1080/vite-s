...
2. vite 相比于 webpack 的优势
    ...
    - 打包流程（开发环境）
      ...
      - webpack 是先把所有模块都解析完，打包，然后交给浏览器
        - 整个打包完成后，才会去打开 index.html 文件（webpack 会启动一个代理服务器去部署资源）
        - 如果我们能拿到打包后的文件，后续就算不要 webpack，也可以在浏览器正常运行
      - vite 是 **按需处理**，只处理浏览器需要的模块，在开发环境，是没有打包这个操作的
        - vite 一开始（大概是在初始化 vite.config.js 配置之后）就会直接启动代理服务器打开 index.html 文件，后续遇到啥模块，就解析啥模块
          - 纯按需要解析，没遇到的模块，是不会提前去做解析的，所以也就没有一个完整的打包文件
          - 浏览器运行的过程，全程需要 vite 的参与（浏览器请求文件时，需要 vite 对此文件模块进行编译解析）
